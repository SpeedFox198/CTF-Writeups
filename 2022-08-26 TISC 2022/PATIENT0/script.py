...
def reverse_callback():
    ...
    c_val, n_val = ord("c"), ord("n")
    def print_permitted_reverse(patch):
            patches = findReverse(desired, calc(patch, accum))
            for last_4_bytes in patches:
                if all(p in permitted_characters for p in last_4_bytes) and last_4_bytes[-1] == n_val:
        ...
        for a in permitted_characters:
            for b in permitted_characters:
                for c in permitted_characters:
                    for d in permitted_characters:
                        print_permitted_reverse((c_val, a, b, c, d))
